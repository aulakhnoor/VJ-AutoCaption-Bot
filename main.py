...
try: app_id = int(os.environ.get("app_id", "22152751"))
except Exception as app_id: print(f"⚠️ App ID Invalid {app_id}")
try: api_hash = os.environ.get("api_hash", "a0212e5f9b4ff167a13cf52eebbd34ec")
except Exception as api_id: print(f"⚠️ Api Hash Invalid {api_hash}")
try: bot_token = os.environ.get("bot_token", "6930747667:AAGqawglSK5rrfqNf2ITIAdAi_xIFRAukGg")
except Exception as bot_token: print(f"⚠️ Bot Token Invalid {bot_token}")
try: custom_caption = os.environ.get("custom_caption", "`{Join Backup Channel ⚠️👇
 Bhot Tagda Contact On the way hy os channal par so join fast
👇👇👇👇👇👇👇
https://t.me/+Wsr76uh5DxNjNzVl}`")
